...
func (cellCvt *CellMicroCalcConverter) CsvToCell() (func(row []string) (interface{}, error), error) {
	...
	// convert group by attributes value to string:
	// for built-in attribute type use ParseBool() or Atoi()
	// for built-in float type return error: group by attribute cannot be float
	// for enum attribute type return enum id by code
	fa := make([]func(src string) (interface{}, error), nGrp+1)
	...
	for k, ga := range aGroupBy {
		...
		switch {
		case !ga.typeOf.IsBuiltIn(): // enum based attribute type: find and return enum id by enum code

			f, err := ga.typeOf.itemCodeToId(msgName, false)
			if err != nil {
				return nil, err
			}
		...
		case ga.typeOf.IsFloat(): // group by attribute cannot be float

			fa[k] = func(src string) (interface{}, error) {
				return nil, errors.New("group by attribute cannot be float type: " + msgName)
			}

		case ga.typeOf.IsBool():
			fa[k] = func(src string) (interface{}, error) { return strconv.ParseBool(src) }
		case ga.typeOf.IsString():
			fa[k] = func(src string) (interface{}, error) { return src, nil }
		case ga.typeOf.IsInt():
			fa[k] = func(src string) (interface{}, error) { return strconv.Atoi(src) }
		default:
			return nil, errors.New("invalid (not supported) entity attribute type: " + msgName)
		}
	}
	// for calculated value use parse float 64 bits
	fa[nGrp] = func(src string) (interface{}, error) { return strconv.ParseFloat(src, 64) }

	// return converter from csv strings into: (run id, calc_id, group by attributes, calculated value attribute)
	cvt := func(row []string) (interface{}, error) {

		cell := CellMicroCalc{Attr: make([]attrValue, nGrp+1)}

		n := len(cell.Attr)
		if len(row) != n+2 {
			return nil, errors.New("invalid size of csv row, expected: " + strconv.Itoa(nGrp+3) + ": " + cellCvt.Name)
		}
		...
		if id, ok := cellCvt.DigestToId[row[0]]; ok {
			...
			return nil, errors.New("invalid (or empty) run digest: " + row[0] + " entity: " + cellCvt.Name)
		}
		if id, ok := cellCvt.CalcNameToId[row[1]]; ok {
			cell.CalcId = id
		} else {
			return nil, errors.New("invalid (or empty) calculation name: " + row[1] + " entity: " + cellCvt.Name)
		}

		// convert group by attributes calculated value attribute
		for k := 0; k < nGrp+1; k++ {

			cell.Attr[k].IsNull = row[k+2] == "" || row[k+2] == "null"
